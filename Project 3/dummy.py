...
def em():
    ...
    # calculate the maximum likelihood of each observation xi
        ...
        if len(log_likelihood) < 2:
            continue
        print("diff=", np.abs(log_value - log_likelihood[-2]))
        if np.abs(log_value - log_likelihood[-2]) < eps:
            ...
            # combined pdf func
            # plt.plot(bins, pdf(bins, means[0], variances[0]) + pdf(bins, means[1], variances[1]) + pdf(bins, means[2],
            #                                                                                            variances[2]),color='green', lw=8, label="True pdf")
            #plt.plot(bins, pdf(bins, mu1, sigma1) + pdf(bins, mu2, sigma2) + pdf(bins, mu3, sigma3), color='red', lw=8,label="True pdf")
            plt.plot(bins, pdf(bins, mu1, sigma1), color='red', label="Cluster 1")
            plt.plot(bins, pdf(bins, mu2, sigma2), color='green', label="Cluster 2")
            plt.plot(bins, pdf(bins, mu3, sigma3), color='yellow', label="Cluster 3")
