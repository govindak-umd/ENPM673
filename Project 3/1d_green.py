...
from find_colour_peaks_green import final_peaks_green_list
from find_colour_peaks_green import k
from find_colour_peaks_green import all_green

X  = np.array (final_peaks_green_list)
k = int(k)
flag = 0
    ...
    # calculate the maximum likelihood of each observation xi
        ...
        if np.abs(log_value - log_likelihood[-2]) < eps:
            ...
            for index in range(k):
                plt.plot(bins, pdf(bins, means[index], variances[index]), color='green')
                # plt.plot(bins, pdf(bins, means[1], variances[1]), color='green', label="Cluster 2")
                # plt.plot(bins, pdf(bins, means[2], variances[2]), color='yellow', label="Cluster 3")
...
def image_testing():
    global k
    global X
    img = cv2.imread('buoy_Green//Test//frame6.png', 1)
    img_rows = img.shape[0]
    img_cols = img.shape[1]

    likelihood_list_test = []
    img_green_channel = img[:, :, 1]
    img_green_channel = np.reshape(img_green_channel,(img_rows*img_cols,))
    print(img_green_channel.shape)
    means, variances, weights = em()
    for j in range(k):
        likelihood_list_test.append(pdf(img_green_channel, means[j], np.sqrt(variances[j])) * weights[j])
...
print(fin.shape)
cv2.imshow("Final output - GREEN", fin)
cv2.waitKey(0)
cv2.destroyAllWindows()