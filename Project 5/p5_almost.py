...
def Triangulation_check(R,T,K,inliers_1,inliers_2):
    R_init = np.eye(3)
    T_init = np.reshape(np.array([0.0,0.0,0.0]),(1,3))
    P_1 = K @ np.hstack((R_init, -R_init@T_init.T))
    P_2 =  K @ np.hstack((R, -R@T))
    X = []
    for j in range(len(inliers_1)):
        x1 = inliers_1[j]
        x2 = inliers_2[j]
        A1 = (x1[0]*P_1[:,2].T)-P_1[:,0].T
        A2 = (x1[1] * P_1[:, 2].T) - P_1[:, 1].T
        A3 = (x2[0] * P_2[:, 2].T) - P_2[:, 0].T
        A4 = (x2[1] * P_2[:, 2].T) - P_2[:, 1].T
        A = [A1,A2,A3,A4]
        U,S,V = np.linalg.svd(A)
        V = V.T
        X.append(V[:,-1])
    ...
    for i in range(X.shape[0]):
        ...
        if R[2] @ np.subtract(new_x[0:3], T) > 0 and new_x[2] > 0:
            return [True,R,T]
        else:
            return [False]
...
# Initiate SIFT detector
    ...
    R_array, T_array = cameraposestimation(E)
    for j in range(3):
        T_j = np.reshape(T_array[j], (3, 1))
        out = Triangulation_check(R_array[j],T_j,K,inliers_1,inliers_2)
        print("out=",out)
        if out[0]==True:
            #print("Here")
            R = out[1]
    ...
    x_nf = Translation[0]
    z_nf = Translation[2]
    list_points_x.append(-x_cf)
    list_points_x.append(-x_nf)
    list_points_y.append(z_cf)
    list_points_y.append(z_nf)

    plt.plot(list_points_x,list_points_y,'o')
    plt.show()
